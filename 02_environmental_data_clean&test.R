...
euclid <- vegdist(covariates_only, method = 'euclidean')

mpa_confounders <- adonis(euclid ~ alt_enforcement, data = mpa_complete, method = 'euclidian')


# test for homogeneity of dispersions (variances)
dispersion <- betadisper(euclid, group = mpa_complete$alt_enforcement )
permutest(dispersion)

plot(dispersion, ellipse = TRUE, hull = FALSE)


nmds <- metaMDS(euclid, distance = 'euclid')
nmds.df <- tibble(
  nmds1 = nmds$points[,1],
  nmds2 = nmds$points[,2],
