...
##  code: Shane Blowes
##==============================================================================================================
# code to calculate alpha-scale metrics (two protection classes)
...
alpha_summary <- mpa3_reduced %>% 
  # first collate the transects at each unique location (combination of lat lon)
  ...
  summarise(
    coverage = iNEXT::DataInfo(abundance)$SC,
...
N_hist <- ggplot() +
  geom_histogram(data = alpha_summary,
                 aes(x = N, fill = alt_enforcement), binwidth = 0.1) +
  scale_fill_grey(name = 'Protection') + 
  scale_x_continuous(trans = 'log10', breaks = c(100, 200, 400, 800, 1600)) +
  labs(x = expression(paste('Number of individuals (per 375',m^2,')')),
       y = 'Number of sites') +
  theme_bw() +
...
coverage_hist <- ggplot() +
  ...
  scale_fill_grey(name = 'Protection') + 
  labs(x = expression(paste(alpha, '-scale sample completeness (coverage)')), 
       y = 'Number of sites') +
  theme_bw() +
...
#      file = '~/Dropbox/1current/mediterranean_mpa/submitted/JApplEcol/revision1/data_results/alpha_scale_revisionData.Rdata')
