stack_plot_low <- function (mob_out, trt_group, prop = FALSE, stretch = TRUE, common_scale = FALSE, 
    ...
    xlab = '', xlab_top = '', ylab = '', main_title = '') 
{
    ...
    N_interp = pracma::pchip(N$effort, N$effect, virt_effort[-length(agg$effort)])
    N_lower = pracma::pchip(N$effort, N$lower, virt_effort[-length(agg$effort)])
    N_upper = pracma::pchip(N$effort, N$upper, virt_effort[-length(agg$effort)])
    SAD = data.frame(type = "SAD", effort = effort, effect = SAD_interp, lower = SAD_lower, upper = SAD_upper)
    N = data.frame(type = "N", effort = effort[-length(agg$effort)], effect = N_interp, lower = N_lower, upper = N_upper)
    dat = rbind(SAD, N, agg)
    
    ylim = with(dat, max(effect, lower, upper))
    ...
    if(plot==TRUE & uncertainty==FALSE){
      ...
      title(main = main_title, adj = 0, line = 5, cex.main = 2)
    	...
    	mtext(side = 3, xlab_top, padj = -3, cex = 1.5)
	    if(legend)
    ...
    if(plot==TRUE & uncertainty==TRUE){
      ...
      polygon(c(unique(dat$effort)[-41], rev(unique(dat$effort)[-41])), 
      ...
      lines(unique(dat$effort)[-41], dat$effect[dat$type == "N"], col = "#d95f02", xaxt=NULL, yaxt=NULL, lwd=3) 
      lines(unique(dat$effort), dat$effect[dat$type == "agg"], col = "#7570b3", xaxt=NULL, yaxt=NULL, lwd=3)    
      abline(h=0, lty=2)
      ticks = axTicks(side = 3)
      n_indices = round(seq(1, length(virt_effort), length.out = length(ticks)))
      axis(side = 3, at = effort[n_indices], labels = round(virt_effort[n_indices]),
           cex.axis = 1.5)
      mtext(side = 3, xlab_top, padj = -2, cex = 1.25)
      if(legend)
        legend(0.3*max(dat$effort), 0.9*max(dat$effect), 
               legend=c('SAD effect', 'N effect', 'Aggregation\neffect'), 
               lty=1, col=c("#1b9e77", "#d95f02", "#7570b3"), bty='n', 
